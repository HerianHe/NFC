...
int nfc_start_social_handler(ke_msg_id_t const msgid,
	...
	co_random_init( (serial_num[2]<<16) + (serial_num[1]<<8) + (serial_num[0]) );
...
int nfc_reader_15_sec_timer_handler(ke_msg_id_t const msgid,
									void const *param,
									ke_task_id_t const dest_id,
									ke_task_id_t const src_id){
	ke_msg_send_basic(NFC_READER_END, TASK_NFC, TASK_NFC);
	return 0;
}
...
int nfc_reader_select_app_handler(ke_msg_id_t const msgid,
	...
	uart2_finish_transfers();
	//receive ack
	uart2_read(rec_buf, 11, nfc_reader_select_app_rec_ack_callback);
	return 0;
}
void nfc_reader_select_app_rec_ack_callback(uint8_t UART_STATUS){
	//receive info
	uart2_read(rec_buf, rec_buf[9] + 2, nfc_reader_select_app_rec_info_callback);
}
void nfc_reader_select_app_rec_info_callback(uint8_t UART_STATUS){
	//receive info
...
const struct ke_msg_handler nfc_reader[] = {
	...
	{NFC_READER_15_SEC_TIMER,	              (ke_msg_func_t)nfc_reader_15_sec_timer_handler},
